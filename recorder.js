// recorder.js - EchoParrot v1.9.0 Beta
// タブ名ファイル名 / 30分時間制限 / シリアルキー認証（AES-GCM暗号化）
...
// ---- シリアルキー設定 ----
// 検証用SHA-256ハッシュ（平文キーはここに書かない）
const VALID_KEY_HASH = '48311e1243f90ec8e2d7b25fbdf35c6a4ddf28fd32bedb1e0347f1f0247eaa51';
...
// AES-GCMキー生成（デバイス固有のsaltを使用）
async function getEncryptionKey() {
  const salt = 'EchoParrot-v1-salt-2026';
  const keyMaterial = await crypto.subtle.importKey(
    'raw', new TextEncoder().encode(salt),
    { name: 'PBKDF2' }, false, ['deriveKey']
  );
  return crypto.subtle.deriveKey(
    { name: 'PBKDF2', salt: new TextEncoder().encode('ep-device-salt'), iterations: 100000, hash: 'SHA-256' },
    keyMaterial,
...
// シリアルキーを暗号化して保存
async function saveEncryptedKey(plainKey) {
  ...
  chrome.storage.local.set({ licenseData: JSON.stringify(data) });
}

// 保存済みキーを復号して返す
async function loadDecryptedKey() {
  return new Promise((resolve) => {
    chrome.storage.local.get(['licenseData'], async (result) => {
      if (!result.licenseData) { resolve(null); return; }
      try {
        const data = JSON.parse(result.licenseData);
        const key  = await getEncryptionKey();
        const iv   = new Uint8Array(data.iv);
        const enc  = new Uint8Array(data.enc);
...
// キーを検証してPRO状態を更新
async function verifyAndSetPro(plainKey) {
  const hash = await sha256(plainKey.trim().toUpperCase());
  return hash === VALID_KEY_HASH;
}

// 起動時にライセンス確認
async function initLicense() {
  const savedKey = await loadDecryptedKey();
  if (savedKey) {
    const valid = await verifyAndSetPro(savedKey);
    if (valid) {
      isPro = true;
      updatePlanBadge();
    } else {
...
btnKeyApply.addEventListener('click', async () => {
  const input = keyInput.value.trim().toUpperCase();
  if (!input) { keyStatus.textContent = 'キーを入力してください'; keyStatus.className = 'key-status err'; return; }

  const valid = await verifyAndSetPro(input);
  if (valid) {
    await saveEncryptedKey(input);
    isPro = true;
    updatePlanBadge();
    keyInput.value = '';
